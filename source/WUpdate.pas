...
begin
  ...
  begin
    ...
    begin
      ...
      if not Assigned(OnStatus) and (UpdateUpdate <> wuuSilent) then
      begin
        //strTmp := PAnsiChar(buf);
        strTmp :=  buf;
        strTmp :=  StrPas(buf);
        strTmp := ArrayToString(buf);
        { TODO -oSVS -cERROR : StrPas(buf) }
        Messagedlg(StrPas(buf),mtError,[mbok],0);    // SVS
      end;
    end;
  end;
end;
...
begin
  ...
  if (ctrlURL = '') or
     not URLGetFile(FHintconnect,ctrlURL,WinTempDir,False) then
  begin
...
begin
  ...
  {$IFNDEF USEUAC}
  RenameFile(srcname, wintempdir + 'wurepl.exe');
  {$ENDIF}
end;
