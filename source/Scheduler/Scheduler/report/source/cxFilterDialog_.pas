...
{$IFDEF DELPHI6}
  Variants,
{$ENDIF}
  Windows, Classes, Controls, ExtCtrls, Forms, StdCtrls, SysUtils, cxButtonEdit,
  ...
  cxTextEdit, cxLookAndFeelPainters, cxRadioGroup;
...
var
  ...
  procedure InitEdits(AComboBox: TcxComboBox; AEdit: TcxCustomEdit;
    AItem: TcxFilterCriteriaItem);
  const
    AOperatorMap: array [TcxFilterOperatorKind] of TcxFilterControlOperator = (
  ...
  var
    AOperator: TcxFilterControlOperator;
  begin
    AOperator := AOperatorMap[AItem.OperatorKind];
    if AItem.ValueIsNull(AItem.Value) then
      case AOperator of
        fcoEqual:
          AOperator := fcoBlanks;
        fcoNotEqual:
          AOperator := fcoNonBlanks;
      end;
      ...
      case AOperator of
        fcoBlanks, fcoNonBlanks:
          begin
            ...
            AValue := Null;
            ADisplayValue := cxGetResourceStringNet(cxSFilterBlankCaption);
          end;
      end;
...
begin
  ...
  if (AFilterEditHelper = nil) or (ACriteria = nil) then
    Exit;
  ...
  try
    ...
    begin
      ...
      if not ACriteria.SupportedLike then
        ASupportedOperators := ASupportedOperators - [fcoLike, fcoNotLike];
      FSupportedOperators := ASupportedOperators;
      Caption := cxGetResourceStringNet(cxSFilterDialogCaption);
      lblTitle.Caption := cxGetResourceStringNet(cxSFilterDialogRows);
      btnOK.Caption := cxGetResourceStringNet(cxSFilterControlDialogActionOkCaption);
      btnCancel.Caption := cxGetResourceStringNet(cxSFilterControlDialogActionCancelCaption);
      lblSingle.Caption := cxGetResourceStringNet(cxSFilterDialogUse) + ' ' +
        ACriteria.UnderscoreWildcard + ' ' + cxGetResourceStringNet(cxSFilterDialogSingleCharacter);
      lblSeries.Caption := cxGetResourceStringNet(cxSFilterDialogUse) + ' ' +
        ACriteria.PercentWildcard + ' ' + cxGetResourceStringNet(cxSFilterDialogCharactersSeries);
      ...
      rbAnd.Caption := cxGetResourceStringNet(cxSFilterDialogOperationAnd);
      rbOr.Caption := cxGetResourceStringNet(cxSFilterDialogOperationOr);
...
procedure TcxFilterDialog.ReadState(Reader: TReader);
var
  AFont: TFont;
begin
  AFont := TFont.Create;
...
begin
  AWildcardChars := [AnsiChar(FCriteria.UnderscoreWildcard),
    AnsiChar(FCriteria.PercentWildcard)];
